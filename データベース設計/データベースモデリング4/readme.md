...
| --------- | ------------ | ----------- |
| id        | INTEGER (PK) | 主キー         |
| team_id   | VARCHAR(50)  | SlackのチームID |
| team_name | VARCHAR(100) | ワークスペース名    |
...
| ------------- | ------------------------- | ------------ |
| id            | INTEGER (PK)              | 主キー          |
| workspace_id  | INTEGER (FK → workspaces) | 所属ワークスペース    |
...
| channel_id       | VARCHAR(50)                   | 送信先のSlackチャンネルID                                               |
| message          | TEXT                          | リマインダーの文面                                                      |
| schedule_value   | INTEGER NULL                  | 周期の具体値（X日おきのX、毎週X曜日のX、毎月X日のX。毎日の場合はNULL）                       |
| next_send_at     | TIMESTAMP                     | 次回送信予定日時。バッチが1時間ごとに動き、現在時刻 >= next_send_at のリマインダーを配信対象として抽出する |
